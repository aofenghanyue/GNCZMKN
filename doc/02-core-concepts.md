...
连续 form 在周期开始的 `publish()` 中刷新 truth/view；连续状态推进由积分器调用 `computeDerivatives()` 完成。机上导航、估计飞行状态或任务化视图应放在 `vehicle.input` / `vehicle.process` 组件中，而不是混入 form truth view。
...
`Perturbation` 是每个 vehicle 自己的拉偏状态源。SimFlow 内置数值 materializer 或单次 mission 只给它输入纯数字 case 值；拉偏组件负责把这些数字解释成其它组件真正需要的状态，例如阻力系数偏置、密度倍率、发动机温度档位对应的资产文件名，或随飞行状态变化的动态偏置。

其它组件不直接读取 SimFlow 配置，也不关心 case 来自单次、矩阵、随机生成还是项目自定义 materializer。它们只在依赖注入后读取同一 vehicle 的 `IPerturbationProvider`。会影响资产加载的拉偏应在组件 `initialize()` 中读取；随飞行状态变化的拉偏应由拉偏组件在 `update()` 中刷新，并由后续 stage 的组件读取。
...
SimFlow 是仿真前置物化层，不是新的仿真内核。它读取 base mission 和 materializer 配置，生成每个 case 的 `effective_mission.json`，然后串行或多进程运行这些普通 mission。多进程时父进程负责生成 case 文件和汇总 exit code，子进程只执行：

```powershell
gnc_sim.exe --config <case>/effective_mission.json
```

因此单个 case 的复现也使用同一条命令。`simflow_summary.csv` 记录每个 case 的执行状态；不需要额外的隐藏运行状态文件才能复现。
...
| SimFlow case 复现需要重新跑 SimFlow | 复现单个 case 直接运行该 case 的 `effective_mission.json` |
