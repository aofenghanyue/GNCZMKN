    ...
    mission << R"json(
{
  ...
  "vehicles": [
    {
      ...
      "input": [
        { "type": "vehicle.input.cartesian_imu_3dof.ideal", "name": "imu", "config": {} },
        { "type": "vehicle.input.cartesian_satellite_nav_3dof.ideal", "name": "satnav", "config": {} },
        { "type": "vehicle.input.cartesian_air_data_3dof.ideal", "name": "air_data", "config": {} },
        {
          "type": "vehicle.input.cartesian_seeker_3dof.ideal",
          "name": "seeker",
          "config": { "target_truth_lookup_name": "target.truth" }
        }
      ],
      "process": [
        { "type": "vehicle.process.cartesian_phase_sequencer_3dof.ideal", "name": "phase", "config": { "phase_name": "boost" } },
        { "type": "vehicle.process.cartesian_trajectory_planner_3dof.ideal", "name": "planner", "config": {} },
        { "type": "vehicle.process.cartesian_navigation_3dof.ideal", "name": "navigation", "config": {} },
        { "type": "vehicle.process.cartesian_target_tracking_3dof.ideal", "name": "tracking", "config": {} },
        { "type": "vehicle.process.cartesian_guidance_3dof.ideal", "name": "guidance", "config": {} },
        { "type": "vehicle.process.cartesian_flight_control_3dof.ideal", "name": "flight_control", "config": {} },
        { "type": "vehicle.process.cartesian_control_allocation_3dof.ideal", "name": "control_allocation", "config": {} }
      ],
      "output": [
      ...
      "interaction": {
        "components": [
          { "type": "interaction.cartesian_3dof.standard", "name": "interaction", "config": {} }
        ]
      }
    },
    {
  ...
  "outputs": {
    ...
    "format": "csv",
    "session_name": "ideal_cartesian_3dof_mission",
    "record": {
  ...
  "termination": {
    "type": "termination.engagement_cartesian_3dof",
    "name": "termination",
    "config": {
      "min_range_m": 1.0,
      "min_altitude_m": -1.0,
  ...
  "summary": {
    "type": "summary.engagement_cartesian_3dof.ideal",
    "name": "summary",
    "config": {}
  }
}
...
void checkGeneratedArtifacts(const fs::path& output_dir,
    ...
    test_support::require(
        summary_text.find("Ideal Cartesian 3DOF Engagement Summary") !=
            std::string::npos,
        "Ideal Cartesian 3DOF engagement summary section was missing.");
    ...
    const std::vector<std::string> required_columns = {
        ...
        "interceptor.interaction.acceleration.x",
...
int main() {
    try {
        ...
        const fs::path output_dir =
            fs::current_path() / "test_outputs" / "ideal_cartesian_3dof_mission";
        std::error_code ec;
        ...
        test_support::require(builder.loadConfigString(missionText(output_dir)),
                              "Ideal Cartesian 3DOF mission JSON did not parse.");
        auto& simulator = builder.build();

        auto* interaction =
            simulator.getRegistry()
                .get<gnc::forms::cartesian_3dof::IInputProvider>(
                    "interceptor.interaction");
        test_support::require(interaction != nullptr,
                              "Cartesian 3DOF standard interaction was not registered.");

        simulator.run();
        test_support::require(
            simulator.getTerminationReason().find("time limit") !=
                std::string::npos,
            "Cartesian 3DOF engagement did not terminate with a time-limit reason.");

        checkGeneratedArtifacts(output_dir, "ideal_cartesian_3dof_mission");

        const fs::path example_output_dir =
            fs::current_path() / "user" / "outputs" /
            "example_06_ideal_cartesian_3dof_baseline";
        ec.clear();
        fs::remove_all(example_output_dir, ec);

        const auto example_mission_path =
            sourceRoot() / "user" / "example_06_ideal_cartesian_3dof_baseline" /
            "config" / "mission.json";
        gnc::core::SimulationBuilder example_builder;
        test_support::require(
            example_builder.loadConfigString(readText(example_mission_path)),
            "Example ideal Cartesian 3DOF mission JSON did not parse.");
        auto& example_simulator = example_builder.build();
        example_simulator.run();
        test_support::require(
            example_simulator.getTerminationReason().find("time limit") !=
                std::string::npos,
            "Example Cartesian 3DOF mission did not terminate with a time-limit reason.");
        checkGeneratedArtifacts(example_output_dir,
                                "ideal_cartesian_3dof_baseline");

        std::cout << "ideal cartesian 3dof mission checks passed\n";
        return 0;
