...
public:
    ...
    std::vector<std::string> getComponentsForInterface(std::type_index type_idx) const {
    ...
    std::vector<std::type_index> getRegisteredInterfaces() const {
        std::vector<std::type_index> result;
        result.reserve(interface_map_.size());
        for (const auto& [type_idx, _] : interface_map_) {
