...
class EngagementTerminationCartesian6Dof final
    : public gnc::core::ComponentBase,
      public gnc::interfaces::ITerminationEvaluator {
public:
    EngagementTerminationCartesian6Dof()
        : ComponentBase("EngagementTerminationCartesian6Dof") {}
    ...
    void update(double) override {
        ...
        const auto& interceptor = interceptor_truth_->getCartesian6DoFTruth();
        ...
        if (stop_on_non_finite_state_ &&
            ...
            reason_ = "non-finite Cartesian 6DOF engagement state";
            return;
        }
...
private:
    static bool isFinite(const gnc::forms::cartesian_6dof::Truth& truth) {
        return truth.state.position_m.allFinite() &&
               truth.state.velocity_mps.allFinite() &&
               std::isfinite(truth.state.attitude_body_to_frame.w()) &&
               std::isfinite(truth.state.attitude_body_to_frame.x()) &&
               std::isfinite(truth.state.attitude_body_to_frame.y()) &&
               std::isfinite(truth.state.attitude_body_to_frame.z()) &&
               truth.state.angular_rate_body_radps.allFinite() &&
               truth.acceleration_mps2.allFinite() &&
               truth.angular_acceleration_body_radps2.allFinite() &&
               std::isfinite(truth.sample_time_s);
    }

    gnc::forms::cartesian_6dof::ITruthView* interceptor_truth_ = nullptr;
