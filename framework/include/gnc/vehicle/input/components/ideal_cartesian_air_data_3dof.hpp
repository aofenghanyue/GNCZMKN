...
class IdealCartesianAirData3Dof final : public gnc::core::ComponentBase,
                                        public IAirData3Dof,
                                        public gnc::interfaces::IObservable {
public:
    IdealCartesianAirData3Dof()
        : ComponentBase("IdealCartesianAirData3Dof") {}
    ...
    void update(double) override {
        ...
        const double altitude_m = truth.state.position_m.z();
        const double speed_mps = truth.state.velocity_mps.norm();
        const auto sample = atmosphere_->sample(altitude_m);
        measurement_.altitude_m = altitude_m;
        measurement_.speed_mps = speed_mps;
        ...
        measurement_.mach_number =
            speed_mps / std::max(1.0, sample.speed_of_sound_mps);
        measurement_.dynamic_pressure_pa =
            0.5 * measurement_.density_kg_per_m3 * speed_mps * speed_mps;
    }
